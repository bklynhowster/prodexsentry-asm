...
def i3_empty_envelopes(runs, floor_pct: int = 90) -> dict:
    """I3 — THE COLLECTOR ORDER. Of the newest heavies on hosts KNOWN to serve 443,
    at least `floor_pct`% must carry a non-empty passive envelope.
    ...
    total = len(runs)
    empty = [r for r in runs if envelope_is_empty(r.get("envelope"))]
    pct = round(100 * (total - len(empty)) / total) if total else 100
    return {"total": total, "empty": empty, "full_pct": pct,
            "ok": total == 0 or pct >= floor_pct, "floor_pct": floor_pct}
...
# ⚠ 443 MUST BE KNOWN OPEN — NO DEFAULT FALLBACK, and that is a DELIBERATE
# difference from demotion_writer.known_ports(), which returns (443, 80, 22) when
# asset_surface has no row. That default is right for "which ports should we
# probe"; it is wrong here, because it would assert an HTTPS invariant against
# every host with no surface data — including the SFTP pair. Different question,
# different answer, said out loud so the next reader does not "unify" them.
# Both arms of the shape are walked (subdomains[].services[] AND
# subdomains[].hosts[].services[]); missing the second would silently shrink the
# population this invariant protects.
Q_I3 = """
with https_assets as (
  select distinct s.asset_id
    from public.asset_surface s,
         lateral jsonb_array_elements(coalesce(s.surface_data->'subdomains', '[]'::jsonb)) sub
   where exists (select 1 from jsonb_array_elements(coalesce(sub->'services', '[]'::jsonb)) svc
                  where svc->>'port' = '443')
      or exists (select 1 from jsonb_array_elements(coalesce(sub->'hosts', '[]'::jsonb)) h,
                             jsonb_array_elements(coalesce(h->'services', '[]'::jsonb)) svc
                  where svc->>'port' = '443')
)
select distinct on (r.asset_id)
  ...
  from scan_run r
  join https_assets h on h.asset_id = r.asset_id
 where r.intensity = 'heavy'
   and r.completed_at > now() - interval '%s days'
...
def run_live(dsn: str) -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            _report("I3", "newest heavy on a 443-open host carries a full passive envelope",
                    r3["ok"], f"{r3['full_pct']}% full of {r3['total']} host(s) "
                              f"(floor {r3['floor_pct']}%)",
                    [v["asset_id"] for v in r3["empty"][:5]], failures)
...
def _selftest() -> int:
    ...
    # I3 — the collector order. The empty envelope is the real 09-03 key set.
    ...
    bad3 = [{"asset_id": f"h{i}", "envelope": empty_env} for i in range(6)] + \
           [{"asset_id": f"g{i}", "envelope": full_env} for i in range(5)]      # 5/11 = 45%
    good3 = [{"asset_id": f"g{i}", "envelope": full_env} for i in range(11)]
    ok &= i3_empty_envelopes(bad3)["ok"] is False and i3_empty_envelopes(bad3)["full_pct"] == 45
    ok &= i3_empty_envelopes(good3)["ok"] is True
    ...
    print(f"  I3 fires at the measured post-cutover 5/11 (45%), passes at 130/130: "
          f"{'ok' if not i3_empty_envelopes(bad3)['ok'] and i3_empty_envelopes(good3)['ok'] else 'FAIL'}")
